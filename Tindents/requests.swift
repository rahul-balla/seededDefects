...
class requests {
   ...
   func getMatches(completionBlock: @escaping ([String:AnyObject]?) -> () ) -> Void {
    ...
    func updateProfileInfoRequest(email:String, name:String, username:String, charge:Int, description:String, completionBlock: @escaping ([String:AnyObject]?) -> () ) -> Void {
        
        let request = NSMutableURLRequest(url: NSURL(string: "http://127.0.0.1:5000/updateProfileInfo")! as URL)
        let session = URLSession.shared
        request.httpMethod = "POST"
        
        var params = ["email":email, "name":name, "username":username, "charge":charge, "description":description] as! Dictionary<String, Any>
